...
Implements [`\l`](SystemCommands/#l-name-load-file-or-directory). 
...
Returns `x` formatted to plain text, as used by the console. Obeys console width and height set by [`\c`](SystemCommands/#c-hw-console-heightwidth).
```q
...
Returns the memory stats from [`\w`](SystemCommands/#w-0-workspace) into a more readable dictionary.
```q
q).Q.w[]
used| 168304
heap| 67108864
