...
connections to C#. This can be found here:
[<span class="underline">http://code.kx.com/wsvn/code/kx/kdb+/c/c.cs</span>](http://code.kx.com/wsvn/code/kx/kdb%2B/c/c.cs)
...
The C# source code for this paper can be found on the Kx GitHub
page:

[https://github.com/kxcontrib/csharpgui](https://github.com/kxcontrib/csharpgui)
...
public c(string h, int p, string u, string pw, int maxBufferSize)
{
    ...
    //This differs from above as we have added the length of the password as well, // plus an extra character to take account of the separator between u and pw ‘:’ J = 0;
    w(u + ":" + pw + "\x3");
...
private void button1_Click(object sender, EventArgs e)
{
    ...
    object result = (object)conn.k("select from trade"); c.Flip table = (c.Flip)result;
...
![Query output](img/image5.png)  
<small>_Query output_</small>


### Building dynamic queries

As the query consists only of a single string value, it is easy to
...
To carry this out, we use a new method called BuildQuery (presented on
the next page) and replace the hard-coded query with:
...
without exposing them to the q-sql language.
...
The GetDaily method utilises the `getSummary` function on our kdb+
...
![Example chart](guis/media/image12.png)  
...
were built with Visual Studio 2010. The C# source code for this paper
can be found at
[github.com/kxcontrib/csharpgui](https://github.com/kxcontrib/csharpgui).
