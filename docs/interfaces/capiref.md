...
The members which are common to all variant types are t,u,r. The field n is common to all variant types which have a length. These may be dereferenced as usual in the c language: 
...
The fields of the variant types which represent an atom (sometimes call a scalar) are
...
e.g. `KB` is the type for a vector of booleans, and the negative, `-KB` is for an atom of type boolean.
...
# Functions 
Unless otherwise specified, all functions accepting `K` objects should not be passed `NULL`.


## b9
`K b9(I mode,K x)`
Serialise `x` using q IPC using `mode` capabilities level.
...
Return Value
On success, byte array K object with serialised representation. On error, `NULL` is returned, and `es` will contain error string.
...
Return Value
On success, deserialized K object. On error, `NULL` is returned, and `es` will contain error string.
...
## dl
`K dl(V* f, I n)`
The dynamic link. Function takes a C function that would take _n_ K objects as arguments and returns a K object.
Shared library only.
...
Return Value
On success, K object with result of `.` application. On error, NULL is returned, and `es` will contain error string. See `ee` for result handling example.
...
Since 3.5 2017.02.16, 3.4 2017.03.13

## Appending to a list
When appending to a list, if the capacity of the list is 
insufficient to accomodate the new data, the list is reallocated with the contents of x updated. The new data is always appended.
...
## jk
`K jk(K* x,K y)` 
Append another K object to a mixed list.
...
## jv
`K jv(K* x,K y)`
Append K list `y` to the first of the same type.
Takes ownership of `y`. 
Returns the value of the contents of x.
...
See more on [message types](java-client-for-q.md#message-types).
Note that `k` call will block until messages is sent/received(`handle!=0`) or evaluated(`handle=0`).
...
## knt
`K knt(J n,K x)`
Makes table `x` to be keyed by `n` first columns.
Takes ownership of arguments. `x` is untouched if n greater or equals to number of columns.
...
## krr
`K krr(const S)`
To signal an error from your C code use the function `krr(S);`. Note that `krr` does not create a copy of the string passed to it. 
...
## ktj
`K ktj(I t,J j)`
Create a timestamp or a timespan object. `t` is either `-KP` or `-KN`.
...
`length` is a non-negative, non-null integer. 
...
## m9
`V m9(V)`
Free up memory allocated for the thread‘s pool `m9()` .`m9()` should be called when the thread is about to complete, freeing up memory allocated for that thread's pool.

## okx
`I okx(K x)` 
Verify that byte vector `x` is a valid q IPC byte stream. Decompressed data only.
`x` is untouched.
...
It is similar to `krr`, but it appends a system error message to the user-provided string before passing it to `krr`. 
...
Return Value
On success, int K object containing `d`. On error, NULL is returned, `d` is closed and `es` will contain error string.
...
Return value is an interned string and  should be used to store string in symbol vector.
...
Return Value
On success, positive file descriptor for established connection. On error, 0 or negative value is returned.
