...
To create the column `parent`, we look up the values in the key column using [`?`](/ref/search/#find "find") and then declare the link using `!` – instead of [`$`](/ref/enums/#enum "enum") as we would for a [foreign key enumeration](http://code.kx.com/q4m3/8_Tables/#852-example-of-simple-foreign-key).
